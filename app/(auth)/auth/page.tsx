...
import { IAttributes } from "@/node_modules/oneentry/dist/base/utils";
  ...
  return (
    <div className="flex min-h-screen mt-7">
      <div className="w-full max-w-3xl mx-auto flex flex-col lg:flex-row p-3">
        <div>
          <div
            className="mb-8 lg:md-12 cursor-pointer"
            onClick={() => router.push("/")}
          >
            <ChevronLeft className="text-gray-500 h-6 w-6 sm:h-8 w-8 border-2 rounded-full p-1" />
          </div>
          ...
          <div>
            ...
            from-purple-600 via-pink-500 to-red-500 bg-clip-text text-transparent pb-3"
            >
              {isSignUp ? "Sign Up" : "Sign In"}
            </h2>
            <p className="text-base sm:text-lg lg:text-xl text-gray-400 mb-6 sm:mb-8">
              {isSignUp
                ? "Join MarieStore today add discover exclusive deals on your favorite products!"
                : "Welcome back to MarieStore! Log in to continue your shopping journey."}
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <form className="space-y-4 sm:space-y-6" onSubmit={handleSubmit}>
              ...
              <div>
                <Button
                  className="w-full bg-gradient-to-r from-purple-500 via-pink-500 to-red-500 hover:to-red-600 text-white text-base sm:text-xl font-bold p-4 sm:p-6 rounded-md shadow-xl transition-colors duration-300 ease-in-out cursor-pointer"
                  disabled={isSubmitting}
                >
                  {isSubmitting ? (
